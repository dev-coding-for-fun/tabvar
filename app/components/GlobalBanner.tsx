...
const GlobalBanner: React.FC = () => {
  ...
  // If user data is not loaded yet, don't render anything.
  if (user === undefined) {
    return null;
  }
  ...
  if (user && (hasAcknowledgedOnline || isLocallyDismissed)) {
    return null;
  }
  ...
  const shouldShowBanner = !user || (!hasAcknowledgedOnline && !isLocallyDismissed) ;
